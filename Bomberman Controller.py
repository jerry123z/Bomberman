...
def game():
    ...
    controllers =[]
    for player in range(len(joysticks)):
...
# Loop
	...
	for player in player_group:
		...
		exec'player' + str(event.joy + 1) +'.move(event.value, grid, bomb_group)'            
            if event.type == pygame.JOYBUTTONDOWN and event.button != 9 :
		#checks if the player is able to place a bomb and spawns one if so
		exec 'boolean = player'+ str(event.joy + 1)+ '.can_bomb()'
		if boolean:
		    exec 'x,y,size = player' + str(event.joy + 1)+'.bomb()'
...
def start_screen():
    ...
    instructions2 = Sprites.Label("Press Start", ((320, 350)))
    instructions3 = Sprites.Label("To move your character use the controller's d-pad", ((320, 180)))
    instructions4 = Sprites.Label("To place a bomb press any other button on the controller",((320,200)))
    instructions5 = Sprites.Label("bomb-up    explosion-up   speed-up   kick-bombs   1-up", ((320, 240)))
    wrong_number = Sprites.Label("", ((320, 430)))
    
    allSprites = pygame.sprite.Group(instructions1, instructions2, instructions3, instructions4, instructions5, wrong_number)
    #images
	...
	for event in pygame.event.get():
		...
		if event.button == 9 and len(joysticks) < 5 and len(joysticks) > 1:
		    keepGoing = False
		# checks if there are 2 - 4 players
		elif event.button == 9 and len(joysticks) > 5 :
		    wrong_number.set_text("Too many joysticks (4 Max) please restart")
		elif event.button == 9 and len(joysticks) < 2 :
		    wrong_number.set_text("Too few joysticks (2 Min) please restart")		
